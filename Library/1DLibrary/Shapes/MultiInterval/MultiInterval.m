classdef MultiInterval < handle
    ...
    methods
        ...
        function ComputeInterpolationMatrices(this,interpGrid)
            % compute interpolation matrices for the same interpolation
            % grid in computational space for each interval
        ...
        function IPU = ComputeInterpolationMatricesUniform(this,dy)
            % compute interpolation matrices onto a uniform grid for each
        ...
        function Interp = ComputeInterpolationMatricesPhys(this,y,interval)
            % compute interpolation matrix onto a given set of physical
            % points; need to determine which interval each point is in
                   
            % allow to specify an interval - useful for discontinuous
            % functions where it really matters which interval you're in
            if(nargin==3)
                InterpInterval = this.Intervals(interval).Interval.ComputeInterpolationMatrixPhys(y);
                InterPol = zeros(length(y),length(this.Pts.y));
                InterPol(:,this.Intervals(interval).PtsMask) = InterpInterval.InterPol;
                ...
                doneMask = false(size(y));
                keepMaskFull = false(size(y));

                % loop through shapes and determine which points lie in the
                % shape and compute the interpolation matrix

                for iInterval = 1:this.nIntervals

                    thisInterval = this.Intervals(iInterval).Interval;

                    % determine which points lie in this interval
                    yTol = 10^-10;
                    yMin = thisInterval.yMin - yTol;
                    if(isprop(thisInterval,'yMax'))
                        yMax = thisInterval.yMax + yTol;
                    else
                        yMax = inf;
                    end

                    % ensure we only deal with points in the interval that
                    % haven't been captured before (deals with intersections)
                    keepMask = (y>=yMin) & (y<=yMax) & ~doneMask;
                    keepMaskFull = keepMaskFull | keepMask;

                    % compute the corresponding interpolation matrix
                    thisIP = thisInterval.ComputeInterpolationMatrixPhys(y(keepMask));

                    % combine all together
                    % act on points in this interval and map to output points
                    % in keepMask
                    InterPol(keepMask,this.Intervals(iInterval).PtsMask) = thisIP.InterPol;
        ...
        function Plot(this,V,opts)
            ...
            % catch if we have a display name so it's only added once,
            % rather than for each interval
            ...
            for iInterval = 1:this.nIntervals
                ...
                h = this.Intervals(iInterval).Interval.plot(Vi,opts);
                hold on
                
                % only set a legend entry for the first interval
                if(iInterval == 1 && addDisplayName)
