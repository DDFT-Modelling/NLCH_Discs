...
% Note: This method is only tested if pts are in a shape of an infinite capillary.
    ...
    % Find the bounds within which the full area lies inside the
    % InfCapillary
    y2SepMin    = min(this.Pts.y2) + abs(min(areaPtsCart.y2_kv))/sin(alpha);
    y2SepMax    = max(this.Pts.y2) - abs(max(areaPtsCart.y2_kv))/sin(alpha);

    % Find the bounds within which the area can intersect with the
    % InfCapillary    
    y2OutMin    = min(this.Pts.y2) - abs(max(areaPtsCart.y2_kv))/sin(alpha);
    y2OutMax    = max(this.Pts.y2) + abs(min(areaPtsCart.y2_kv))/sin(alpha);
    
    % Find the points where the area (1) intersectes with the bottom, (2)
    % lies fully within the domain, (3) intersects with the top
    mark_Y2{1}  = ((pts.y2    < y2SepMin) & (pts.y2 >= y2OutMin)) ;
    mark_Ykv{1} = ((pts.y2_kv < y2SepMin) & (pts.y2_kv >= y2OutMin)) ;
    
    mark_Y2{2}  = ((pts.y2    >= y2SepMin) & (pts.y2  <= y2SepMax));
    mark_Ykv{2} = ((pts.y2_kv >= y2SepMin) & (pts.y2_kv <= y2SepMax));            

    mark_Y2{3}  = ((pts.y2    > y2SepMax) & (pts.y2 <= y2OutMax)) ;
    mark_Ykv{3} = ((pts.y2_kv > y2SepMax) & (pts.y2_kv <= y2OutMax)) ;
