classdef PeriodicTanBox < SpectralFourier
    ...
    properties        
        ...
        L2
        alpha1
        Int1, Int2 % added, should be in @Shape but trying to localise changes
        I1, I2 % Integration vectors
    end
    
    methods        
        function this = PeriodicTanBox(Geometry)
        ...
        function d  = GetDistance(this,pts_y1,pts_y2)             
            ...
            y1 = pts_y1;              % spectral
           % y2 = mod(pts_y2,this.L2); % Fourier
            y2 = mod(pts_y2 + this.L2/2,this.L2) - this.L2/2;
        ...
        function M_IntConv = ComputeIntegrationConvolutionMatrix(this,fInt,fConv,shapeParams)
            ...
            % In velocity, so match all mesh parameters
            geom1.N = this.N1;
            geom1.yMin = this.y1Min;
            geom1.yMax = this.y1Max;
            geom1.alpha1 = this.alpha1; % Choose spacing of points
            geom1.alpha2 = 0; % Cluster points at origin
            sLine = SpectralLineTan(geom1);
            ...
            % shapeParams gives cutoff if required
        ...
        function [I1, I2] = Compute1DIntegrationVectors(this)
            geom1.N = this.N1;
            geom1.yMin = this.y1Min;
            geom1.yMax = this.y1Max;
            geom1.alpha1 = this.alpha1; % Choose spacing of points
    ...
    methods (Access = public)
        function [y1,dy1,dx,ddx,dddx,ddddx] = PhysSpace1(this,x1)      
            % Add alpha1 alpha2 as L1 and 0
            [y1,dy1,dx,ddx,dddx,ddddx] = TanIntervalMap(x1,this.alpha1,0,this.y1Min,this.y1Max);
        end
        function xf = CompSpace1(this,y1)
            xf  = InvTanIntervalMap(y1,this.alpha1,0,this.y1Min,this.y1Max);
        end       
