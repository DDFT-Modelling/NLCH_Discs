classdef MultiShape < handle
    ...
    methods
        ...
        function ComputeInterpolationMatrices(this,interpGrid1,interpGrid2)
            ...
            if(nargin==1)
                if(this.periodic(1))
                    ...
                    interpGrid1 = (-1:0.02:1)';
                end
                if(this.periodic(2))
                    interpGrid2 = (0:0.01:1)';
                else
                    interpGrid2 = (-1:0.02:1)';
                end                
            elseif(nargin==2)
            ...
            for iShape = 1:this.nShapes
                ...
                if(isa(this.Shapes(iShape).Shape,'Disc'))
                    IPi = this.Shapes(iShape).Shape.ComputeInterpolationMatrix((0:0.01:1)',(0:0.01:1)',true,true);
                else
        ...
        %------------------------------------------------------------------        
        function Interp = ComputeInterpolationMatricesPhys(this,Pts,shape)
                       
            % points to interpolate onto
            Y1 = Pts.y1_kv;
            Y2 = Pts.y2_kv;

            % allow to specify a shape - useful for discontinuous
            % functions where it really matters which shape you're in
            if(nargin==3)
                
                thisShape = this.Shapes(shape).Shape;
                
                if(strcmp(thisShape.polar,'polar'))
                    Origin = thisShape.Origin;
                    [Theta,R] = cart2pol(Y1-Origin(1),Y2-Origin(2));
                    Theta = mod(Theta,2*pi);
                    [X1,X2] = thisShape.CompSpace(R,Theta);
                else
                    [X1,X2] = thisShape.CompSpace(Y1,Y2);
                end

                InterPolShape = zeros(length(X1),thisShape.M);
                for iPoint = 1:length(X1)
                    % InterpShape.InterPol has size 1 x thisShape.M
                    InterpShape = thisShape.ComputeInterpolationMatrix(X1(iPoint),X2(iPoint));
                    InterPolShape(iPoint,:) = InterpShape.InterPol;
                end

                % combine all together
                % act on points in this shape and map to output points
                InterPol = zeros(length(Y1),length(this.Pts.y1_kv));
                InterPol(:,this.Shapes(shape).PtsMask) = InterPolShape;
                ...
                for iShape = 1:this.nShapes
                    ...
                    % get max and min of x1 and x2; deal with some special
                    % cases, particularly spectral-polar where the points don't
                    % span the full range of permissible inputs to the physical
                    % map
                    if(isa(thisShape,'Disc'))
                        x1Min = 0;
                        x1Max = 1;
                        x2Min = 0;
                        x2Max = 1;    
                    elseif(isa(thisShape,'InfAnnulus'))
                        x1Min = -1;
                        x1Max = 1;
                        x2Min = 0;
                        x2Max = 1;
                    ...
                    InterPolShape = zeros(nPoints,thisShape.M);
        ...
        % all of them
           ...
           firstPlot = true;
                    ...
                    if(plotIJ(iShape,jShape))
                        ...
                        if(firstPlot)
                            plot(y1(PtsMask),y2(PtsMask),'Color',colour,'Linewidth',linewidth, ...
                                 'Linestyle',linestyle,'DisplayName','Intersections');
                            firstPlot = false;
                        else
                            plot(y1(PtsMask),y2(PtsMask),'Color',colour,'Linewidth',linewidth, ...
                                 'Linestyle',linestyle,'HandleVisibility','off');
                        end
                    end
                end
            end
            ...
            % fix axes
            if(isfield(optsDetail,'setAxes'))
                setAxes = optsDetail.setAxes;
            else
                setAxes = true;
            end
            if(setAxes)
